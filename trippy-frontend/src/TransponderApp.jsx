...
function CreatePage() {
  ...
  const buildLeg = () => {
    ...
    return {
      ...
      depart_time: (bType === "train" && tLookupResult?.origin) ? (tLookupResult.origin.actual || tLookupResult.origin.scheduled || (tDate && tDepart ? `${tDate}T${tDepart}:00Z` : null)) : (tDate && tDepart ? `${tDate}T${tDepart}:00Z` : (tDate ? `${tDate}T08:00:00Z` : null)),
            ...
            {tLookupResult && (
              <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 6, border: "1px solid #d4628a55", background: "#d4628a11" }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px 16px" }}>
                  {[["CARRIER", tLookupResult.carrier], ["TRAIN", tLookupResult.number], ["DEP", formatTime(tLookupResult.origin?.actual || tLookupResult.origin?.scheduled)], ["ARR", formatTime(tLookupResult.destination?.actual || tLookupResult.destination?.scheduled)]].map(([l, v]) => (
