...
function DetailPage({ tripId }) {
  ...
  const [bTrainNum, setBTrainNum] = useState(""); const [bTrainLoading, setBTrainLoading] = useState(false); const [bTrainResult, setBTrainResult] = useState(null); const [bTrainErr, setBTrainErr] = useState(null); const [bTrainOptions, setBTrainOptions] = useState(null); const [bTrainCarrier, setBTrainCarrier] = useState(""); const [bTrainNo, setBTrainNo] = useState("");
  const resetBuilder = () => {
    ...
    setBFN(""); setBLoading(false); setBAF(null); setBErr(null); setBFlightOptions(null); setBHN(""); setBHC(""); setBHI(defDate || sd); setBHO(ed); setBHPlace(null); setBO(""); setBD(""); setBDt(defDate || sd); setBTm(""); setBOPlace(null); setBDPlace(null); setBTrainNum(""); setBTrainLoading(false); setBTrainResult(null); setBTrainErr(null); setBTrainOptions(null); setBTrainCarrier(""); setBTrainNo("");
  };
  ...
  const selectTrainDetail = (t) => { setBTrainResult(t); setBTrainOptions(null); setBO(t.origin?.name || ""); setBD(t.destination?.name || ""); if (t.date) setBDt(t.date); const depIso = t.origin?.actual || t.origin?.scheduled || t.hub_stop?.actual_departure || t.hub_stop?.scheduled_departure; if (depIso) setBTm(depIso.substring(11, 16)); setBOPlace(t.origin?.lat ? { lat: t.origin.lat, lng: t.origin.lng, city: t.origin.name, name: t.origin.name } : null); setBDPlace(t.destination?.lat ? { lat: t.destination.lat, lng: t.destination.lng, city: t.destination.name, name: t.destination.name } : null); setBTrainCarrier(t.carrier || ""); setBTrainNo(t.number || ""); };
  const handleTrainQuery = async () => { if (!bTrainNum.trim()) return; setBTrainLoading(true); setBTrainErr(null); setBTrainResult(null); setBTrainOptions(null); try { const r = await api(`/trains/lookup?number=${encodeURIComponent(bTrainNum.trim().toUpperCase())}${bDt ? `&date=${bDt}` : ""}`); const trains = r.trains || []; if (trains.length === 1) { selectTrainDetail(trains[0]); } else if (trains.length > 1) { setBTrainOptions(trains); } else { setBTrainErr("NO MATCH \u2014 verify train number"); } } catch (e) { const msg = e?.message || ""; if (msg.includes("429") || msg.toLowerCase().includes("rate")) { setBTrainErr("STAND BY \u2014 too many lookups. Try again shortly."); } else if (msg.includes("503") || msg.includes("not configured")) { setBTrainErr("API credentials not configured \u2014 check DB_CLIENT_ID / SNCF_API_KEY"); } else { setBTrainErr("NO MATCH \u2014 verify train number (ICE/RE/EC = DB, TGV/TER = SNCF)"); } } setBTrainLoading(false); };
  ...
  const addLeg = async () => {
    ...
    else { const oLat = bOPlace?.lat || 0; const oLng = bOPlace?.lng || 0; const dLat = bDPlace?.lat || 0; const dLng = bDPlace?.lng || 0; const oCity = bOPlace?.city || bO; const dCity = bDPlace?.city || bD; const trainDepart = bTrainResult?.origin?.actual || bTrainResult?.origin?.scheduled || bTrainResult?.hub_stop?.actual_departure || bTrainResult?.hub_stop?.scheduled_departure; const trainArrive = bTrainResult?.destination?.actual || bTrainResult?.destination?.scheduled; newLeg = { type: bType, origin: { code: bO.slice(0, 3).toUpperCase(), city: oCity, lat: oLat, lng: oLng }, destination: { code: bD.slice(0, 3).toUpperCase(), city: dCity, lat: dLat, lng: dLng }, depart_time: (bType === "train" && trainDepart) ? trainDepart : `${bDt}T${bTm || "08:00"}:00Z`, arrive_time: (bType === "train" && trainArrive) ? trainArrive : `${bDt}T12:00:00Z`, carrier: bType === "train" ? (bTrainCarrier || "Train") : "Bus", vehicle_number: bType === "train" ? (bTrainNo || null) : null, metadata: {} }; }
    try {
          ...
          {showLegBuilder ? (
            <div className="border rounded" style={{ background: "var(--bg-surface)", borderColor: "var(--border-primary)" }}>
              ...
              <div className="p-3">
                ...
                {bType === "train" && (<div className="rounded border p-2.5 mb-2" style={{ background: "var(--bg-card)", borderColor: "var(--border-primary)" }}><p style={{ fontFamily: FONT, fontSize: "8px", letterSpacing: "2px", color: "#d4628a", marginBottom: 6 }}>TRAIN NUMBER LOOKUP</p><div className="flex flex-col sm:flex-row gap-2 mb-2"><div className="flex-1"><Label>TRAIN NUMBER</Label><Input type="text" value={bTrainNum} onChange={e => { setBTrainNum(e.target.value); setBTrainResult(null); setBTrainErr(null); }} onKeyDown={e => e.key === "Enter" && handleTrainQuery()} placeholder="ICE123, TGV6180..." style={{ textTransform: "uppercase", letterSpacing: "1px" }} /></div><div className="flex-1"><Label>DATE</Label><DatePicker value={bDt} onChange={setBDt} /></div><div className="flex items-end"><button onClick={handleTrainQuery} disabled={bTrainLoading || !bTrainNum.trim()} className="w-full sm:w-auto px-4 py-2.5 rounded text-xs font-bold tracking-widest" style={{ background: "var(--bg-surface)", color: bTrainNum.trim() ? "#d4628a" : "var(--text-tertiary)", border: "1px solid var(--border-primary)", fontFamily: FONT, fontSize: "9px" }}>{bTrainLoading ? <Spinner /> : "QUERY"}</button></div></div>{bTrainResult && (<div className="rounded border p-2.5 mb-2" style={{ background: "var(--bg-surface)", borderColor: "#d4628a" }}><div className="flex items-center gap-2 mb-1"><span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style={{ background: "#d4628a" }} /><span className="relative inline-flex rounded-full h-2 w-2" style={{ background: "#d4628a" }} /></span><span style={{ color: "#d4628a", fontFamily: FONT, fontSize: "9px", fontWeight: 700 }}>MATCH · {bTrainResult.status?.toUpperCase()}</span></div><div className="grid grid-cols-2 gap-x-4 gap-y-0.5">{(() => { const depTime = bTrainResult.origin?.actual || bTrainResult.origin?.scheduled || bTrainResult.hub_stop?.actual_departure || bTrainResult.hub_stop?.scheduled_departure; const arrTime = bTrainResult.destination?.actual || bTrainResult.destination?.scheduled; const depLabel = bTrainResult.hub_stop && !bTrainResult.origin?.scheduled ? `DEP (${bTrainResult.hub_stop.name?.split('(')[0].trim()})` : "DEP"; return [["CARRIER", bTrainResult.carrier], ["TRAIN", bTrainResult.number], [depLabel, formatTime(depTime)], ["ARR", formatTime(arrTime)]].map(([l, v]) => (<div key={l} className="flex items-baseline gap-1.5"><span style={{ color: "var(--text-secondary)", fontFamily: FONT, fontSize: "8px", minWidth: 40 }}>{l}</span><span style={{ color: "var(--text-primary)", fontFamily: FONT, fontSize: "9px" }}>{v || "—"}</span></div>)); })()}</div></div>)}{bTrainOptions && (<div className="mb-2"><p style={{ fontFamily: FONT, fontSize: "8px", letterSpacing: "2px", color: "#d4628a", marginBottom: 6 }}>{bTrainOptions.length} TRAINS FOUND — SELECT ONE</p><div style={{ display: "flex", flexDirection: "column", gap: 4 }}>{bTrainOptions.map((opt, i) => (<button key={i} onClick={() => selectTrainDetail(opt)} className="tappable-card" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "8px 10px", borderRadius: 4, border: "1px solid var(--border-primary)", background: "var(--bg-surface)", cursor: "pointer", textAlign: "left" }}><div><span style={{ fontFamily: FONT, fontSize: "11px", fontWeight: 600, color: "#d4628a" }}>{opt.number}</span><span style={{ fontFamily: FONT, fontSize: "9px", color: "var(--text-secondary)", marginLeft: 8 }}>{opt.carrier}</span></div><span style={{ fontFamily: FONT, fontSize: "9px", color: "var(--text-secondary)" }}>{formatTime(opt.origin?.actual || opt.origin?.scheduled || opt.hub_stop?.actual_departure) || "—"} → {formatTime(opt.destination?.actual || opt.destination?.scheduled) || "—"}</span></button>))}</div></div>)}{bTrainErr && <p style={{ fontFamily: FONT, fontSize: "8px", color: "#e84233", marginTop: 4 }}>{bTrainErr}</p>}<p style={{ fontFamily: FONT, fontSize: "8px", color: "var(--text-tertiary)", marginTop: 4 }}>Or enter details manually below</p></div>)}
...
function CreatePage() {
  ...
  // Train lookup
  const [tLookupNum, setTLookupNum] = useState(""); const [tLookupLoading, setTLookupLoading] = useState(false); const [tLookupResult, setTLookupResult] = useState(null); const [tLookupErr, setTLookupErr] = useState(null); const [tLookupOptions, setTLookupOptions] = useState(null);
  ...
  const resetBuilder = () => {
    ...
    setTOrigin(""); setTDest(""); setTOPlace(null); setTDPlace(null); setTDepart(""); setTArrive(""); setTOperator(""); setTNumber(""); setTDate(defDate); setTLookupNum(""); setTLookupLoading(false); setTLookupResult(null); setTLookupErr(null); setTLookupOptions(null);
    setValErrors([]);
  };
  ...
  const selectTrainFromLookup = (t) => { setTLookupResult(t); setTLookupOptions(null); setTOrigin(t.origin?.name || ""); setTDest(t.destination?.name || ""); if (t.date) setTDate(t.date); const depIso = t.origin?.actual || t.origin?.scheduled || t.hub_stop?.actual_departure || t.hub_stop?.scheduled_departure; if (depIso) setTDepart(depIso.substring(11, 16)); const arrIso = t.destination?.actual || t.destination?.scheduled; if (arrIso) setTArrive(arrIso.substring(11, 16)); setTOPlace(t.origin?.lat ? { lat: t.origin.lat, lng: t.origin.lng, city: t.origin.name, name: t.origin.name } : null); setTDPlace(t.destination?.lat ? { lat: t.destination.lat, lng: t.destination.lng, city: t.destination.name, name: t.destination.name } : null); setTOperator(t.carrier || ""); setTNumber(t.number || ""); };
  const handleTrainLookup = async () => { if (!tLookupNum.trim()) return; setTLookupLoading(true); setTLookupErr(null); setTLookupResult(null); setTLookupOptions(null); try { const r = await api(`/trains/lookup?number=${encodeURIComponent(tLookupNum.trim().toUpperCase())}${tDate ? `&date=${tDate}` : ""}`); const trains = r.trains || []; if (trains.length === 1) { selectTrainFromLookup(trains[0]); } else if (trains.length > 1) { setTLookupOptions(trains); } else { setTLookupErr("NO MATCH \u2014 verify train number"); } } catch (e) { const msg = e?.message || ""; if (msg.includes("429") || msg.toLowerCase().includes("rate")) { setTLookupErr("STAND BY \u2014 too many lookups. Try again shortly."); } else if (msg.includes("503") || msg.includes("not configured")) { setTLookupErr("API credentials not configured \u2014 check DB_CLIENT_ID / SNCF_API_KEY"); } else { setTLookupErr("NO MATCH \u2014 verify train number (ICE/RE/EC = DB, TGV/TER = SNCF)"); } } setTLookupLoading(false); };
  ...
  const buildLeg = () => {
    ...
    return {
      ...
      depart_time: (bType === "train" && tLookupResult) ? (tLookupResult.origin?.actual || tLookupResult.origin?.scheduled || tLookupResult.hub_stop?.actual_departure || tLookupResult.hub_stop?.scheduled_departure || (tDate && tDepart ? `${tDate}T${tDepart}:00Z` : null)) : (tDate && tDepart ? `${tDate}T${tDepart}:00Z` : (tDate ? `${tDate}T08:00:00Z` : null)),
      arrive_time: (bType === "train" && tLookupResult?.destination) ? (tLookupResult.destination.actual || tLookupResult.destination.scheduled || (tDate && tArrive ? `${tDate}T${tArrive}:00Z` : null)) : (tDate && tArrive ? `${tDate}T${tArrive}:00Z` : (tDate ? `${tDate}T12:00:00Z` : null)),
    ...
    <div className="px-4 sm:px-6 py-4" style={{ maxWidth: isDesktop ? 580 : "36rem", margin: "0 auto", padding: isDesktop ? "32px 24px 60px" : undefined }}>
      ...
      {bType === "train" && (
        <div>
          {/* Train number lookup */}
          <div style={{ border: "1px solid var(--border-primary)", borderRadius: 6, padding: "10px 12px", background: "var(--bg-card)", marginBottom: 10 }}>
            <p style={{ fontFamily: FONT, fontSize: "8px", letterSpacing: "2px", color: "#d4628a", marginBottom: 6 }}>TRAIN NUMBER LOOKUP</p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              <input type="text" value={tLookupNum} onChange={e => { setTLookupNum(e.target.value); setTLookupResult(null); setTLookupErr(null); }} onKeyDown={e => e.key === "Enter" && handleTrainLookup()} placeholder="ICE123, TGV6180..." style={{ ...bInput({ flex: "1 1 120px", textTransform: "uppercase", letterSpacing: "1px", background: "var(--bg-surface)" }) }} />
              <input type="date" value={tDate} onChange={e => setTDate(e.target.value)} style={{ ...bInput({ flex: "0 0 140px", background: "var(--bg-surface)" }) }} />
              <button onClick={handleTrainLookup} disabled={tLookupLoading || !tLookupNum.trim()} style={{ height: 40, padding: "0 14px", background: "#d4628a22", color: "#d4628a", borderRadius: 6, border: "1px solid #d4628a55", fontFamily: FONT, fontSize: "9px", letterSpacing: "2px", fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap" }}>
                {tLookupLoading ? <Spinner /> : "QUERY"}
              </button>
            </div>
            <p style={{ fontFamily: FONT, fontSize: "8px", color: "var(--text-tertiary)", marginTop: 6 }}>ICE/RE/EC = Deutsche Bahn · TGV/TER = SNCF · Or enter manually below</p>
            {tLookupErr && <p style={{ fontFamily: FONT, fontSize: "8px", color: "#e84233", marginTop: 4 }}>{tLookupErr}</p>}
            {tLookupOptions && (
              <div style={{ marginTop: 8 }}>
                <p style={{ fontFamily: FONT, fontSize: "8px", letterSpacing: "2px", color: "#d4628a", marginBottom: 6 }}>{tLookupOptions.length} TRAINS FOUND — SELECT ONE</p>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {tLookupOptions.map((opt, i) => (
                    <button key={i} onClick={() => selectTrainFromLookup(opt)} className="tappable-card" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 12px", borderRadius: 6, border: "1px solid var(--border-primary)", background: "var(--bg-surface)", cursor: "pointer", textAlign: "left" }}>
                      <div>
                        <span style={{ fontFamily: FONT, fontSize: "12px", fontWeight: 600, color: "#d4628a", letterSpacing: "1px" }}>{opt.number}</span>
                        <span style={{ fontFamily: FONT, fontSize: "9px", color: "var(--text-secondary)", marginLeft: 10 }}>{opt.carrier}</span>
                      </div>
                      <div style={{ textAlign: "right" }}>
                        <span style={{ fontFamily: FONT, fontSize: "10px", color: "var(--text-primary)" }}>{formatTime(opt.origin?.actual || opt.origin?.scheduled) || "—"} → {formatTime(opt.destination?.actual || opt.destination?.scheduled) || "—"}</span>
                        {opt.date && <span style={{ fontFamily: FONT, fontSize: "8px", color: "var(--text-tertiary)", marginLeft: 8 }}>{opt.date}</span>}
                      </div>
                    </button>
                  ))}
                </div>
            ...
            {tLookupResult && (
              <div style={{ marginTop: 8, padding: "8px 10px", borderRadius: 6, border: "1px solid #d4628a55", background: "#d4628a11" }}>
                <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 4 }}>
                  <span className="relative flex h-2 w-2"><span className="animate-ping absolute inline-flex h-full w-full rounded-full opacity-75" style={{ background: "#d4628a" }} /><span className="relative inline-flex rounded-full h-2 w-2" style={{ background: "#d4628a" }} /></span>
                  <span style={{ fontFamily: FONT, fontSize: "9px", fontWeight: 700, color: "#d4628a" }}>MATCH · {tLookupResult.status?.toUpperCase()}</span>
                </div>
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "2px 16px" }}>
                  {(() => { const depTime = tLookupResult.origin?.actual || tLookupResult.origin?.scheduled || tLookupResult.hub_stop?.actual_departure || tLookupResult.hub_stop?.scheduled_departure; const arrTime = tLookupResult.destination?.actual || tLookupResult.destination?.scheduled; const depLabel = tLookupResult.hub_stop && !tLookupResult.origin?.scheduled ? `DEP (${tLookupResult.hub_stop.name?.split('(')[0].trim()})` : "DEP"; return [["CARRIER", tLookupResult.carrier], ["TRAIN", tLookupResult.number], [depLabel, formatTime(depTime)], ["ARR", formatTime(arrTime)]]; })().map(([l, v]) => (
                    <div key={l} style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                      <span style={{ fontFamily: FONT, fontSize: "8px", color: "var(--text-secondary)", minWidth: 40 }}>{l}</span>
                      <span style={{ fontFamily: FONT, fontSize: "9px", color: "var(--text-primary)" }}>{v || "—"}</span>
                    </div>
                  ))}
                </div>
              </div>
